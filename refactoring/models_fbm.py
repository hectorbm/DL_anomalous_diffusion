...
class FBM:
    ...
    @classmethod
    def brownian(cls, use_exact_exp=False):
        if use_exact_exp:
            model = FBM(hurst_exp=0.5)
        else:
            random_brownian_hurst_exp = np.random.uniform(low=0.42, high=0.58)
            model = FBM(hurst_exp=random_brownian_hurst_exp)
        return model
