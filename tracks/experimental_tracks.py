...
class ExperimentalTracks(tracks.Tracks):
    ...
    def compute_sequences_res_time(self):
        ...
        for i in range(len(self.seq_initial_frame)):
            self.seq_res_time.append((self.seq_final_frame[i] - self.seq_initial_frame[i]) * (1 / FILE_fps))
    ...
    def compute_confinement_regions(self):
        ...
        for i in range(len(self.seq_initial_frame)):
            if self.track_states[self.seq_initial_frame[i]] == 1:
                x = self.axes_data[str(0)][self.seq_initial_frame[i]: self.seq_final_frame[i]]
                y = self.axes_data[str(1)][self.seq_initial_frame[i]: self.seq_final_frame[i]]
                dist_x = max(x) - min(x)
                dist_y = max(y) - min(y)
