...
from generators import generator_second_layer,axis_adaptation_to_net,generate_batch_of_samples_l2
from tools.analysis_tools import plot_confusion_matrix_for_layer
import datetime
import numpy as np

def train_l2_net(batchsize, steps, T, sigma, model_id):
    ...
    x5 = Conv1D(f,6,padding='causal',activation='relu',kernel_initializer=initializer)(inputs)
    x5 = BatchNormalization()(x5)
    x5 = Conv1D(f,6,dilation_rate=2,padding='causal',activation='relu',kernel_initializer=initializer)(x5)
    x5 = BatchNormalization()(x5)
    x5 = Conv1D(f,6,dilation_rate=4,padding='causal',activation='relu',kernel_initializer=initializer)(x5)
    x5 = BatchNormalization()(x5)
    ...
    dense = Dense(615,activation='relu')(con)
    ...
    model.fit(
            x=generator_second_layer(batchsize=batchsize,track_length=steps,track_time=T,sigma=sigma),
            steps_per_epoch=4000,
            epochs=50,
            callbacks=callbacks,
            validation_data=generator_second_layer(batchsize=batchsize,track_length=steps,track_time=T,sigma=sigma),
            validation_steps=10)
    return model
...
def validate_test_data_over_model(model,n_axes,track_length,time_length,sigma):
    test_batchsize = 1000
    axis_data_diff, ground_truth = generate_batch_of_samples_l2(test_batchsize,track_length,time_length,sigma)
...
if __name__ == "__main__":
    #For testing
    train_l2_net(batchsize=64,steps=100,T=1.2,sigma=0,model_id='second_layer_1')    
