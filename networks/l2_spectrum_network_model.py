...
from networks.generators import generator_second_layer_spectrum, axis_adaptation_to_net_spectrum
...
class L2NetworkSpectrumModel(network_model.NetworkModel):
    output_categories = 3
    output_categories_labels = ["H<0.4", "0.4<H<0.6", "H>0.6"]
    model_name = 'L2 Network(Spectrum)'
    ...
    def train_network(self, batch_size):
        ...
        l1_spectrum_keras_model = Model(inputs=inputs, outputs=output_network)

        optimizer = Adam(lr=1e-4)
        l1_spectrum_keras_model.compile(optimizer=optimizer, loss='categorical_crossentropy',
                                        metrics=['categorical_accuracy'])
        l1_spectrum_keras_model.summary()
        ...
        history_training = l1_spectrum_keras_model.fit(
            x=generator_second_layer_spectrum(batch_size=batch_size, track_length=self.track_length,
                                              track_time=self.track_time),
            steps_per_epoch=3000,
            epochs=5,
            callbacks=callbacks,
            validation_data=generator_second_layer_spectrum(batch_size=batch_size, track_length=self.track_length,
                                                            track_time=self.track_time),
            validation_steps=100)

        self.keras_model = l1_spectrum_keras_model
